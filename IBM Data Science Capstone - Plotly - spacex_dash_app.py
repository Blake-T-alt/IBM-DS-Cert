...
OptionsList = [{'label':i,'value':i} for i in spacex_df['Launch Site'].unique()]
OptionsList.insert(0,{'label':'ALL','value':'ALL'})
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(df1, values='class count', names='class', title=f"Launch Success Rate for site {entered_site}")
        return fig
...
def get_scatter(entered_site, slider_range):
    ...
    if entered_site == 'ALL':
        ...
        title=f'Successes by Launch Site and Payload')
        return fig
    else:
